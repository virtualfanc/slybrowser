...
param(
    ...
    [string]$TrustedServiceUrl,
    [string]$CanaryAuthorizationFile,
    [string]$CanaryCacheRoot,
    [string]$CanaryTrustedServiceUrl,
    [string]$CanaryLicenseKeyId,
    [string]$CanaryLicensePublicKeyHex,
    [string]$CanaryReleaseKeyId,
    [string]$CanaryReleasePublicKeyBase64url,
...
if ($authorizationMode) {
    foreach ($name in @('CacheRoot', 'LicenseKeyId', 'LicensePublicKeyHex', 'ReleaseKeyId', 'ReleasePublicKeyBase64url', 'TrustedServiceUrl', 'CanaryAuthorizationFile', 'CanaryCacheRoot', 'CanaryTrustedServiceUrl')) {
    ...
    $canaryAuthorizationPath = (Resolve-Path -LiteralPath $CanaryAuthorizationFile).Path
    $cachePath = [System.IO.Path]::GetFullPath($CacheRoot)
    $canaryLicenseKeyIdValue = if ($CanaryLicenseKeyId) { $CanaryLicenseKeyId } else { $LicenseKeyId }
    $canaryLicensePublicKeyHexValue = if ($CanaryLicensePublicKeyHex) { $CanaryLicensePublicKeyHex } else { $LicensePublicKeyHex }
    $canaryReleaseKeyIdValue = if ($CanaryReleaseKeyId) { $CanaryReleaseKeyId } else { $ReleaseKeyId }
    $canaryReleasePublicKeyBase64urlValue = if ($CanaryReleasePublicKeyBase64url) { $CanaryReleasePublicKeyBase64url } else { $ReleasePublicKeyBase64url }
} else {
...
if ($authorizationMode) {
    $nodeArguments += @(
        ...
        '--trusted-service-url', $TrustedServiceUrl,
        '--canary-authorization-file', $canaryAuthorizationPath,
        '--canary-cache-root', $CanaryCacheRoot,
        '--canary-trusted-service-url', $CanaryTrustedServiceUrl,
        '--canary-license-key-id', $canaryLicenseKeyIdValue,
        '--canary-license-public-key-hex', $canaryLicensePublicKeyHexValue,
        '--canary-release-key-id', $canaryReleaseKeyIdValue,
        '--canary-release-public-key-base64url', $canaryReleasePublicKeyBase64urlValue
    )
    if ($AllowInsecureLocalhost) { $nodeArguments += '--allow-insecure-localhost' }
} else {
...
try {
    ...
    if ($authorizationMode) {
        ...
        if ($AllowInsecureLocalhost) { $pythonArguments += '--allow-insecure-localhost' }
    } else {
...
if ($authorizationMode) {
    $mavenArguments += @(
        ...
        "-Dslybrowser.integration.trustedServiceUrl=$TrustedServiceUrl",
        "-Dslybrowser.integration.canaryAuthorizationFile=$canaryAuthorizationPath",
        "-Dslybrowser.integration.canaryCacheRoot=$CanaryCacheRoot",
        "-Dslybrowser.integration.canaryTrustedServiceUrl=$CanaryTrustedServiceUrl",
        "-Dslybrowser.integration.canaryLicenseKeyId=$canaryLicenseKeyIdValue",
        "-Dslybrowser.integration.canaryLicensePublicKeyHex=$canaryLicensePublicKeyHexValue",
        "-Dslybrowser.integration.canaryReleaseKeyId=$canaryReleaseKeyIdValue",
        "-Dslybrowser.integration.canaryReleasePublicKeyBase64url=$canaryReleasePublicKeyBase64urlValue"
    )
    if ($AllowInsecureLocalhost) { $mavenArguments += '-Dslybrowser.integration.allowInsecureLocalhost=true' }
} else {
...
$previousIntegrationAllowInsecureLocalhost = $env:SLYBROWSER_INTEGRATION_ALLOW_INSECURE_LOCALHOST
$previousIntegrationTrustedServiceUrl = $env:SLYBROWSER_INTEGRATION_TRUSTED_SERVICE_URL
$previousIntegrationCanaryAuthorization = $env:SLYBROWSER_INTEGRATION_CANARY_AUTHORIZATION_FILE
$previousIntegrationCanaryCacheRoot = $env:SLYBROWSER_INTEGRATION_CANARY_CACHE_ROOT
$previousIntegrationCanaryTrustedServiceUrl = $env:SLYBROWSER_INTEGRATION_CANARY_TRUSTED_SERVICE_URL
$previousIntegrationCanaryLicenseKeyId = $env:SLYBROWSER_INTEGRATION_CANARY_LICENSE_KEY_ID
$previousIntegrationCanaryLicensePublicKeyHex = $env:SLYBROWSER_INTEGRATION_CANARY_LICENSE_PUBLIC_KEY_HEX
$previousIntegrationCanaryReleaseKeyId = $env:SLYBROWSER_INTEGRATION_CANARY_RELEASE_KEY_ID
$previousIntegrationCanaryReleasePublicKeyBase64url = $env:SLYBROWSER_INTEGRATION_CANARY_RELEASE_PUBLIC_KEY_BASE64URL
try {
    ...
    $env:SLYBROWSER_INTEGRATION_ALLOW_INSECURE_LOCALHOST = if ($AllowInsecureLocalhost) { '1' } else { $null }
    $env:SLYBROWSER_INTEGRATION_TRUSTED_SERVICE_URL = if ($authorizationMode) { $TrustedServiceUrl } else { $null }
    $env:SLYBROWSER_INTEGRATION_CANARY_AUTHORIZATION_FILE = if ($authorizationMode) { $canaryAuthorizationPath } else { $null }
    $env:SLYBROWSER_INTEGRATION_CANARY_CACHE_ROOT = if ($authorizationMode) { $CanaryCacheRoot } else { $null }
    $env:SLYBROWSER_INTEGRATION_CANARY_TRUSTED_SERVICE_URL = if ($authorizationMode) { $CanaryTrustedServiceUrl } else { $null }
    $env:SLYBROWSER_INTEGRATION_CANARY_LICENSE_KEY_ID = if ($authorizationMode) { $canaryLicenseKeyIdValue } else { $null }
    $env:SLYBROWSER_INTEGRATION_CANARY_LICENSE_PUBLIC_KEY_HEX = if ($authorizationMode) { $canaryLicensePublicKeyHexValue } else { $null }
    $env:SLYBROWSER_INTEGRATION_CANARY_RELEASE_KEY_ID = if ($authorizationMode) { $canaryReleaseKeyIdValue } else { $null }
    $env:SLYBROWSER_INTEGRATION_CANARY_RELEASE_PUBLIC_KEY_BASE64URL = if ($authorizationMode) { $canaryReleasePublicKeyBase64urlValue } else { $null }
    ...
    $env:SLYBROWSER_INTEGRATION_ALLOW_INSECURE_LOCALHOST = $previousIntegrationAllowInsecureLocalhost
    $env:SLYBROWSER_INTEGRATION_TRUSTED_SERVICE_URL = $previousIntegrationTrustedServiceUrl
    $env:SLYBROWSER_INTEGRATION_CANARY_AUTHORIZATION_FILE = $previousIntegrationCanaryAuthorization
    $env:SLYBROWSER_INTEGRATION_CANARY_CACHE_ROOT = $previousIntegrationCanaryCacheRoot
    $env:SLYBROWSER_INTEGRATION_CANARY_TRUSTED_SERVICE_URL = $previousIntegrationCanaryTrustedServiceUrl
    $env:SLYBROWSER_INTEGRATION_CANARY_LICENSE_KEY_ID = $previousIntegrationCanaryLicenseKeyId
    $env:SLYBROWSER_INTEGRATION_CANARY_LICENSE_PUBLIC_KEY_HEX = $previousIntegrationCanaryLicensePublicKeyHex
    $env:SLYBROWSER_INTEGRATION_CANARY_RELEASE_KEY_ID = $previousIntegrationCanaryReleaseKeyId
    $env:SLYBROWSER_INTEGRATION_CANARY_RELEASE_PUBLIC_KEY_BASE64URL = $previousIntegrationCanaryReleasePublicKeyBase64url
}
