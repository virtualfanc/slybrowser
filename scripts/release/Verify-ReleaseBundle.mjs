...
function read7zEntryNames(artifactPath, content) {
  if (!content.subarray(0, 6).equals(Buffer.from([0x37, 0x7a, 0xbc, 0xaf, 0x27, 0x1c]))) return [];
  const configured = process.env.SLYBROWSER_7Z_PATH;
  const candidates = [configured, ...(process.platform === "win32" ? ["7z.exe", "7z"] : ["7zz", "7z"])].filter(Boolean);
  let lastError;
  for (const executable of candidates) {
    try {
      const output = execFileSync(executable, ["l", "-slt", artifactPath], {
        encoding: "utf8", maxBuffer: 16 * 1024 * 1024, windowsHide: true, stdio: ["ignore", "pipe", "pipe"],
      });
      const entries = output.includes("----------") ? output.split("----------").slice(1).join("----------") : output;
      return entries.split(/\r?\n/).filter((line) => line.startsWith("Path = ")).map((line) => line.slice(7).replaceAll("\\", "/"));
    } catch (error) {
      lastError = error;
      if (error?.code !== "ENOENT") break;
    }
  }
  throw new Error(`Unable to inspect 7z release artifact${lastError?.code === "ENOENT" ? ": 7z or 7zz is required" : ""}`);
}
function readArchiveEntryNames(artifactPath, content, archiveFormat) {
  return archiveFormat === "7z" ? read7zEntryNames(artifactPath, content) : readZipEntryNames(content);
}
function assertPublicArtifactSafety(content, entryNames) {
  const text = entryNames.length > 0 ? entryNames.join("\n") : content.toString("latin1");
  const checks = [
...
function assertRequiredLegalResources(artifact, archiveContent, entryNames) {
  const archiveIndex = entryNames.length > 0 ? entryNames.join("\n") : archiveContent.toString("latin1").replaceAll("\\", "/");
...
const [archiveFile, browserFile, driverFile] = await Promise.all([
  identity(artifactPath), identity(resolve(required("--browser"))), identity(resolve(required("--driver"))),
]);
...
const archiveEntryNames = readArchiveEntryNames(artifactPath, archiveFile.content, artifact.archiveFormat);
assertPublicArtifactSafety(archiveFile.content, archiveEntryNames);
...
assertRequiredLegalResources(artifact, archiveFile.content, archiveEntryNames);
