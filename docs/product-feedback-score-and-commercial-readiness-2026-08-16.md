...
2. The website now uses the saved paired 40-entry run: **SlyBrowser 75.73 versus stock
   Chromium 72.11** on coverage-adjusted score. The browsers are Chromium 148 and 153,
   so this is directional operational evidence rather than a same-major fingerprint or
   TLS claim.
...
| Metric | SlyBrowser 148.0.7778.179 | Stock Chromium / Playwright 153.0.8003.0 | Difference |
| --- | ---: | ---: | ---: |
...
| --- | ---: | ---: | ---: | --- | --- |
| Offer and package contract | 15% | 90% | 13.50 | Free/Launch/Studio/Fleet/Grid prices and 1/5/20/200/2,000 concurrency match contracts, service and website. Same browser-quality baseline is documented. | Validate willingness-to-pay and clarify Free commercial-use rights, support/SLA and burst rules. |
...
| Billing and entitlement lifecycle | 20% | 35% | 7.00 | Hosted PayNow test checkout, HMAC/timestamp/store verification, idempotency and activation/renewal/cancel ledger exist. | Anonymous checkout remains unlinked; no automatic subscription-to-license reconciliation; refund/chargeback events are unsupported; production webhook is not deployed. |
| Operations, legal and launch controls | 20% | 15% | 3.00 | Deployment samples, topology and privacy boundary are documented. | Binary terms, privacy/AUP/refund/tax/support ownership, production monitoring/backups/key rotation/DR, shared PostgreSQL authority, multi-node load tests and incident process are incomplete. |
| **Total launch-gate readiness** | **100%** | — | **54.25 / 100** | Strong local engineering foundation. | Not ready for public paid launch. |
...
- Refund/chargeback and disputed-payment policy enforcement.
...
4. Confirm launch platforms: Windows only, Windows+Linux/Docker, or also macOS arm64.
