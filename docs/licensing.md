...
- **Runtime lease**: an Ed25519-signed short-lived envelope bound to one runtime
  session, selected browser release and paid feature set.
...
browser binary may run paid features.
...
- encrypted/authenticated v2 generated for all paid public launch files;
...
- patched browsers should fail paid features when native activation, heartbeat
...
- then server policy falls back to the effective Free entitlement or suspended
  state, depending on the approved commercial policy.
...
- N and N+1 concurrency tests pass for Free, Launch, Studio, Fleet and Grid;
...
- paid features require a signed runtime lease and native activation;
