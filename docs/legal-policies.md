...
- Supported paid plans are Launch, Studio, Fleet and Grid. Fleet and Grid are self-serve SKUs.
...
- A completed full refund immediately ends the paid entitlement for that order, shortens `paidThrough` to the refund completion time, closes active runtime sessions on their next heartbeat and leaves only Free-plan access available.
