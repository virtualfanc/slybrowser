...
It is not production-ready. Legacy private compatibility evidence covered a broader
29-group parser surface, but the first public release exposes 27 supported profile
groups and rejects proxy routing, proxy alignment, GEO/geolocation, and WebRTC
proxy/replace modes. Thirteen stable groups are verified end to end. The largest
remaining blockers are two inherited parser-only fields and other product features,
license enforcement and service work, incomplete full Chromium tests, third-party
license-scan failures, and unverified protected-service results.
...
| Native profile parser tests | 6/6 legacy private parser checks; v1 public contract exposes the supported 27-group subset | PASS |
| Native profile handoff E2E | 27 v1 groups configured; 13 stable groups asserted across browser, window, Worker, and network | PASS |
...
| Profile configuration | Proxy, locale, timezone, viewport, GeoIP, WebRTC, storage and more | First public release exposes 27 supported groups and explicitly excludes proxy routing, proxy alignment, GEO/geolocation, and WebRTC proxy/replace modes | Historical 29-group mapping remains private future-scope evidence, not a v1 public contract |
...
| Font/GEO diagnostics | Implemented | GEO diagnostics are outside first-release scope | Future |
...
1. Harden native profile runtime coverage for the v1 contract. `--sly-config-file`
   now consumes and maps the supported first-release profile groups. Proxy
   credentials/bypass, proxy alignment, browser-process geolocation override, and
   WebRTC proxy/replace modes are future-scope work and must stay rejected in v1
   SDK launches. Add runtime consumers for inherited parser-only `media`/`mac` only
   if a real product requirement exists, and extend component-specific E2E coverage.
   Device scale and complete WebRTC shutdown are not legacy profile capabilities and
...
12. Add extension workflow, font checks, Docker packaging, and then the optional
    human-interaction and Widevine layers. Native GeoIP/proxy/WebRTC proxy tests
    belong to a later product scope and must remain opt-in future coverage.
