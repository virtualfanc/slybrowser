...
Applications provide profile values through the shared SDK user API. The SDK and
browser validate them before creating a profile. Unknown fields, invalid types,
unsafe ranges, and mixed flat/nested profiles fail closed.
...
support. All four bindings use the fields below under `profile`. See
[SDK user API](user-api.md) for the common launch shape and defaults.
...
`profile_secret_forbidden`, or `profile_internal`. Authorization is supplied through
the separate authorization-file argument, never inside `profile`.
