...
The authorization file contains a long-lived random API key and is a credential. The
database stores only an HMAC-SHA256 hash of the secret using a server-only pepper. The
lease and release manifests use separate Ed25519 key pairs. Clients contain only the
two public-key sets. Artifact responses require the session token, do not redirect,
and are bound to the exact SHA-256 selected for that session.
...
SLY_LICENSE_DB=C:\sly\state\license.sqlite
SLY_LICENSE_ADMIN_TOKEN=<random admin bearer token>
SLY_LICENSE_HOST=127.0.0.1
SLY_LICENSE_PORT=8787
SLY_LICENSE_SESSION_TTL_SECONDS=600
SLY_LICENSE_HEARTBEAT_SECONDS=60
```
...
The offline CLI creates the entitlement and writes a new credential file with exclusive
creation semantics. Paid plans require a future paid-through timestamp.
...
Never send an authorization file through logs or browser command-line arguments. If a
file is exposed, revoke the entitlement and issue a new one; the current schema does
not rotate only the secret for an existing license ID.
...
slybrowser install --authorization C:\secure\account_123.authorization.json
```
...
Version selection is explicit: `latest`, `exact`, or `at-or-before`; only the last
policy is a rollback. During an authorized launch both SDKs verify and expose the full
`requested → selected → downloaded → launched` audit. The downloaded and launched
versions must exactly equal the selected signed release; a mismatch closes the browser
and fails with a stable version-chain error instead of falling back.

## Deployment limits and hardening

The checked-in store uses SQLite `BEGIN IMMEDIATE`, WAL and a busy timeout. It provides
atomic capacity enforcement for one authoritative service node and is covered through
the 2,000-session Grid boundary. Do not put independent SQLite databases behind a load
balancer. Before multi-node production, replace the store with PostgreSQL transactions
and row/advisory locking, then run simultaneous N/N+1 tests from multiple nodes.
