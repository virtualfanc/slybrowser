...
This document is the public-safe index for anti-abuse, feature-gate and plan-concurrency security
...
| Frontend plan tamper | Checkout intent ignores user-supplied price/URL fields and only accepts canonical `plan_id`. | Private authorization-service `frontend-plan-tamper` test case | Re-run after live PayNow field samples are confirmed. | Covered locally. |
...
| Old lease replay | Runtime reservation binds startup/device inputs and rejects replayed/mismatched sessions. | Private authorization-service `old-lease-replay` test case | Add native process-copy and IPC replay tests. | Covered locally; native replay still pending. |
| Activation ticket replay | Activation ticket is one-time and cannot be reused after activation. | Private authorization-service `activation-ticket-replay` test case | Run same condition through browser/WebDriver startup. | Covered locally. |
| Clock rollback | Lease generation remains monotonic when service time moves backwards. | Private authorization-service `clock-rollback` test case | Add native watchdog time-skew tests. | Covered locally. |
| Plan feature and concurrency gating | Free can reserve WebDriver, Playwright and Puppeteer sessions, but only one concurrent browser runtime is allowed; downgrade preserves granted automation and clamps active sessions to the Free concurrency limit. | Private authorization-service `plan-concurrency-boundary` test case | Verify the native layer preserves granted automation, rejects stale/revoked feature bits and exits boundedly when concurrency or entitlement checks fail. | Covered locally; native enforcement pending. |
