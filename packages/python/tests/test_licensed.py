...
class LicensedReleaseTests(unittest.TestCase):
    ...
    def test_reads_redacted_online_license_info_without_creating_session(self) -> None:
        ...
        self.assertEqual(info.plan, "basic")
        self.assertEqual(info.effective_plan, "basic")
    ...
    def test_heartbeat_delay_uses_service_interval(self) -> None:
        self.assertEqual(_heartbeat_delay_seconds(120, lambda: 0), 120)
        self.assertEqual(_heartbeat_delay_seconds(120, lambda: 1), 120)
        self.assertEqual(_heartbeat_delay_seconds(120, lambda: 0.5), 120)
        self.assertEqual(_heartbeat_delay_seconds(5, lambda: 1), 5)
        self.assertEqual(_heartbeat_delay_seconds(0, lambda: 1), 1)
    ...
    def test_verifies_downloads_extracts_and_reuses_cache(self) -> None:
        ...
        self.assertEqual(grant.plan, "basic")
    ...
    def test_prompts_for_renewal_when_paid_plan_has_expired(self) -> None:
        client = Fixture(
            session_error=True,
            session_error_code="license_plan_expired",
            session_error_status=403,
        ).client()
        with self.assertRaises(LicenseServiceError) as raised:
            client.create_session(platform="windows", arch="x64")
        self.assertEqual(raised.exception.code, "license_plan_expired")
        self.assertEqual(raised.exception.status, 403)
        self.assertRegex(str(raised.exception), r"(?i)paid plan has expired.*renew")
