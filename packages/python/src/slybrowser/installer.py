...
_MAXIMUM_7Z_ENTRIES = 100_000
_INSPECTION_TIMEOUT_SECONDS = 30
_EXTRACTION_TIMEOUT_SECONDS = 5 * 60
...
def _safe_extract_7z(archive: Path, destination: Path, maximum_expanded_bytes: int) -> None:
    configured = os.environ.get("SLYBROWSER_7Z_PATH")
    executable = next((value for value in (configured, "7zz", "7z") if value and (Path(value).is_file() or shutil.which(value))), None)
    if executable is None:
        raise ArtifactError("7z or 7zz is required to extract browser archives", code="artifact_extractor_missing")
    try:
        ...
        _inspect_7z_listing(inspection.stdout, maximum_expanded_bytes)
        subprocess.run(
            [executable, "x", str(archive), f"-o{destination}", "-y", "-bd", "-bb0"],
            check=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT,
            text=True,
        ...
        raise ArtifactError("Browser archive extraction timed out", code="artifact_extract_timeout") from error
    except subprocess.CalledProcessError as error:
        raise ArtifactError("Unable to extract browser artifact", code="artifact_extract_failed") from error
    expanded = 0
...
def _inspect_7z_listing(listing: str, maximum_expanded_bytes: int) -> None:
    separator_match = re.search(r"^----------\s*$", listing, re.MULTILINE)
    separator = separator_match.end() if separator_match else None
    if separator is None:
        raise ArtifactError("Unable to inspect browser archive", code="artifact_extract_failed")
    ...
    for record in records:
        ...
        raw_name = fields.get("Path")
        if not raw_name:
            continue
        entries += 1
        if entries > _MAXIMUM_7Z_ENTRIES:
            raise ArtifactError("Browser archive contains too many entries", code="artifact_layout_invalid")
        normalized = raw_name.replace("\\", "/")
        parts = normalized.split("/")
        if normalized.startswith("/") or (len(normalized) >= 2 and normalized[1] == ":") or ".." in parts or "\x00" in normalized:
            raise ArtifactError("Browser archive contains an unsafe path", code="artifact_layout_invalid")
        identity = normalized.casefold() if os.name == "nt" else normalized
        if identity in seen:
            raise ArtifactError("Browser archive contains duplicate paths", code="artifact_layout_invalid")
        seen.add(identity)
        attributes = fields.get("Attributes", "")
        if "Symbolic Link" in fields or attributes.lower().startswith("l") or " reparse " in attributes.lower():
            raise ArtifactError("Browser archive contains a symbolic link", code="artifact_layout_invalid")
        size_text = fields.get("Size", "0")
        if not size_text.isdecimal():
            raise ArtifactError("Browser archive has an invalid entry size", code="artifact_extract_failed")
        expanded += int(size_text)
        if expanded > maximum_expanded_bytes:
            raise ArtifactError("Browser archive expands beyond its allowed size", code="artifact_expanded_too_large")
    if entries == 0:
        raise ArtifactError("Browser archive contains no entries", code="artifact_layout_invalid")


def _safe_extract_archive(archive: Path, destination: Path, archive_format: str, maximum_expanded_bytes: int) -> None:
    if archive_format == "7z":
        _safe_extract_7z(archive, destination, maximum_expanded_bytes)
    else:
        _safe_extract(archive, destination, maximum_expanded_bytes)
...
def install_granted_browser(
    ...
    *,
    ...
    try:
        ...
        archive = downloads / f"{grant.artifact.sha256}.{grant.artifact.archiveFormat}"
        try:
        ...
        if extractor:
            ...
            _safe_extract_archive(archive, temporary_directory, grant.artifact.archiveFormat, maximum)
