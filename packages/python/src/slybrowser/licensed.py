...
MAX_BOOTSTRAP_HEARTBEAT_FAILURES = 10
...
def _heartbeat_delay_seconds(
    ...
    _ = random_float
    return max(1.0, float(heartbeat_after_seconds))
    ...
    def _run(self) -> None:
        while not self.stop_event.wait(_heartbeat_delay_seconds(self.grant.heartbeat_after_seconds)):
            try:
                ...
                if self.consecutive_failures >= MAX_BOOTSTRAP_HEARTBEAT_FAILURES:
                    self.stop_event.set()
...
def prepare_latest_authorized_browser(
    ...
    *,
    ...
    try:
        ...
        if heartbeat.failure and heartbeat.consecutive_failures >= MAX_BOOTSTRAP_HEARTBEAT_FAILURES:
