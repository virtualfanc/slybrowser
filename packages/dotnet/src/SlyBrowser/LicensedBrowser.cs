...
internal sealed class LicensedLaunchSettings
{
    public LicenseServiceClientOptions Trust { get; init; } = OfficialTrust.Create();
...
internal static class LicensedBrowser
{
    private const int MaxBootstrapHeartbeatFailures = 10;
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (heartbeat.Failure is not null &&
                heartbeat.ConsecutiveFailures >= MaxBootstrapHeartbeatFailures)
        ...
        private async Task LoopAsync()
        {
            while (!_stop.IsCancellationRequested)
            {
                ...
                catch (Exception exception)
                {
                    Failure = exception;
                    ConsecutiveFailures++;
                    if (ConsecutiveFailures >= MaxBootstrapHeartbeatFailures) return;
                }
            }
        }
