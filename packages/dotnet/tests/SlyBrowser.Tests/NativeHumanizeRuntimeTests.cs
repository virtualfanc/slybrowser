...
public sealed class NativeHumanizeRuntimeTests
{
    ...
    public async Task ScoresNativeHumanizeRuntimeWhenConfigured()
    {
        ...
        object negativeCanary = await ArtifactMismatchCanaryAsync(authorization, settings);
    ...
    private static bool AuthorizationConfigured(string? authorization) =>
        ...
        Environment.GetEnvironmentVariable("SLYBROWSER_INTEGRATION_TRUSTED_SERVICE_URL") is not null &&
        Environment.GetEnvironmentVariable("SLYBROWSER_INTEGRATION_CANARY_AUTHORIZATION_FILE") is not null &&
        Environment.GetEnvironmentVariable("SLYBROWSER_INTEGRATION_CANARY_CACHE_ROOT") is not null &&
        Environment.GetEnvironmentVariable("SLYBROWSER_INTEGRATION_CANARY_TRUSTED_SERVICE_URL") is not null;

    private static LicensedLaunchSettings LicensedSettings(WebDriverLaunchSettings webdriver) =>
        LicensedSettings(
            webdriver,
            Environment.GetEnvironmentVariable("SLYBROWSER_INTEGRATION_CACHE_ROOT")!,
            Environment.GetEnvironmentVariable("SLYBROWSER_INTEGRATION_TRUSTED_SERVICE_URL")!);

    private static LicensedLaunchSettings LicensedSettings(
        WebDriverLaunchSettings webdriver,
        string cacheRoot,
        string trustedServiceUrl,
        bool canaryTrust = false) =>
        new()
        {
            Trust = new LicenseServiceClientOptions
            {
                ...
                {
                    [TrustEnvironment(canaryTrust, "SLYBROWSER_INTEGRATION_CANARY_LICENSE_KEY_ID", "SLYBROWSER_INTEGRATION_LICENSE_KEY_ID")] =
                        Convert.FromHexString(TrustEnvironment(canaryTrust, "SLYBROWSER_INTEGRATION_CANARY_LICENSE_PUBLIC_KEY_HEX", "SLYBROWSER_INTEGRATION_LICENSE_PUBLIC_KEY_HEX")),
                },
                ReleaseTrustedKeys = new Dictionary<string, byte[]>
                {
                    [TrustEnvironment(canaryTrust, "SLYBROWSER_INTEGRATION_CANARY_RELEASE_KEY_ID", "SLYBROWSER_INTEGRATION_RELEASE_KEY_ID")] =
                        Base64UrlDecode(TrustEnvironment(canaryTrust, "SLYBROWSER_INTEGRATION_CANARY_RELEASE_PUBLIC_KEY_BASE64URL", "SLYBROWSER_INTEGRATION_RELEASE_PUBLIC_KEY_BASE64URL")),
                },
                HttpClient = LocalhostIntegrationHttpClient(canaryTrust),
                AllowInsecureLocalhost = Environment.GetEnvironmentVariable("SLYBROWSER_INTEGRATION_ALLOW_INSECURE_LOCALHOST") == "1",
                TrustedServiceUrls = new HashSet<string>
                {
                    trustedServiceUrl,
                },
            },
    ...
    private static HttpClient? LocalhostIntegrationHttpClient(bool canaryTrust)
    {
        if (!canaryTrust || Environment.GetEnvironmentVariable("SLYBROWSER_INTEGRATION_ALLOW_INSECURE_LOCALHOST") != "1") return null;
        string? trustedThumbprint = Environment.GetEnvironmentVariable("SLYBROWSER_INTEGRATION_CANARY_TLS_THUMBPRINT");
        if (string.IsNullOrWhiteSpace(trustedThumbprint)) return null;
        return new HttpClient(new HttpClientHandler
        {
            ...
            ServerCertificateCustomValidationCallback = (request, certificate, _, _) =>
                ...
                string.Equals(certificate.GetCertHashString(), trustedThumbprint, StringComparison.OrdinalIgnoreCase),
        });
    }

    private static async Task<object> ArtifactMismatchCanaryAsync(
        string? authorization,
        WebDriverLaunchSettings webdriver)
    {
        if (authorization is null) return new { id = "not-configured", status = "NOT_EVALUATED" };
        LicensedLaunchSettings settings = LicensedSettings(
            webdriver,
            Path.Combine(Environment.GetEnvironmentVariable("SLYBROWSER_INTEGRATION_CANARY_CACHE_ROOT")!, "dotnet"),
            Environment.GetEnvironmentVariable("SLYBROWSER_INTEGRATION_CANARY_TRUSTED_SERVICE_URL")!,
            true);
        ArtifactException error = await Assert.ThrowsAsync<ArtifactException>(() =>
            LicensedBrowser.InstallAuthorizedAsync(
                Environment.GetEnvironmentVariable("SLYBROWSER_INTEGRATION_CANARY_AUTHORIZATION_FILE")!,
                settings,
                TestContext.Current.CancellationToken));
        Assert.Equal("artifact_runtime_hash_mismatch", error.Code);
        return new
        {
            id = "reject-mismatched-browser-driver",
            status = "PASS",
            observedCode = error.Code,
        };
    }

    private static string TrustEnvironment(bool canaryTrust, string canaryName, string primaryName) =>
        canaryTrust
            ? Environment.GetEnvironmentVariable(canaryName) ?? Environment.GetEnvironmentVariable(primaryName)!
