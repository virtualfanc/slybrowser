...
public sealed class LicensedReleaseTests
{
    ...
    {
        Assert.Equal(
            typeof(SlyBrowserClient).GetMethod("LaunchLatestAsync", BindingFlags.NonPublic | BindingFlags.Static)!.ReturnType,
            typeof(SlyBrowserClient).GetMethod("LaunchAuthorizedAsync", BindingFlags.NonPublic | BindingFlags.Static)!.ReturnType);
        Assert.Equal(
            typeof(SlyBrowserClient).GetMethod("InstallLatestAsync", BindingFlags.NonPublic | BindingFlags.Static)!.ReturnType,
            typeof(SlyBrowserClient).GetMethod("InstallAuthorizedAsync", BindingFlags.NonPublic | BindingFlags.Static)!.ReturnType);
        Assert.Equal(
            typeof(SlyBrowserClient).GetMethod("PrepareLatestAuthorizedBrowserAsync", BindingFlags.NonPublic | BindingFlags.Static)!.ReturnType,
            typeof(SlyBrowserClient).GetMethod("PrepareAuthorizedBrowserAsync", BindingFlags.NonPublic | BindingFlags.Static)!.ReturnType);
    }
        ...
        try
        {
            ...
            await File.WriteAllTextAsync(
                ...
                JsonSerializer.Serialize(LicenseDocument(
                    ...
                    secretOverrides: new Dictionary<string, object?> { ["plan"] = "ultra" })),
                TestContext.Current.CancellationToken);
    ...
    public async Task VerifiesDownloadsExtractsReusesCacheAndReleases()
    {
        ...
        try
        {
            ...
            await File.WriteAllTextAsync(Path.Combine(root, "downloads", $"{fixture.ArtifactSha256}.7z"), "corrupt-archive", TestContext.Current.CancellationToken);
    ...
    public async Task ReadsRedactedOnlineLicenseInfoWithoutCreatingSession()
    {
        ...
        Assert.Equal("basic", info.Plan);
        Assert.Equal("basic", info.EffectivePlan);
    ...
    [Fact]
    public void DotNetBootstrapHeartbeatUsesServiceInterval()
    {
        Assert.Equal(TimeSpan.FromSeconds(120), LicensedBrowser.HeartbeatDelay(120));
        Assert.Equal(TimeSpan.FromSeconds(5), LicensedBrowser.HeartbeatDelay(5));
    ...
    [Fact]
    public async Task PromptsForRenewalWhenPaidPlanHasExpired()
    {
        Fixture fixture = new(
            sessionError: true,
            sessionErrorCode: "license_plan_expired",
            sessionErrorStatus: HttpStatusCode.Forbidden);
        string root = Path.Combine(Path.GetTempPath(), $"sly-dotnet-expired-{Guid.NewGuid():N}");
        Directory.CreateDirectory(root);
        try
        {
            string authorization = await fixture.WriteAuthorizationAsync(root);
            LicenseServiceException error = await Assert.ThrowsAsync<LicenseServiceException>(
                () => SlyBrowserClient.InstallLatestAsync(
                    ...
                    fixture.Settings(root),
                    TestContext.Current.CancellationToken));
            Assert.Equal("license_plan_expired", error.Code);
            Assert.Equal(403, error.Status);
            Assert.Matches("(?i)paid plan has expired.*renew", error.Message);
        }
        finally
        {
            if (Directory.Exists(root)) Directory.Delete(root, recursive: true);
        ...
        private HttpResponseMessage Handle(HttpRequestMessage request)
        {
            ...
            if (url.EndsWith("/v2/runtime/sessions", StringComparison.Ordinal) && request.Method == HttpMethod.Post)
            {
                ...
                Dictionary<string, object?> ticket = new()
                {
                    ...
                    ["artifactUrl"] = $"https://api.slybrowser.test/v1/releases/artifacts/{_artifactSha256}.7z",
                };
                return Json(new Dictionary<string, object?>
                {
                    ["schemaVersion"] = 2,
            ...
            Dictionary<string, object?> unsigned = new()
            {
                ...
                ["artifacts"] = new[]
                {
                    new Dictionary<string, object?>
                    {
                        ["platform"] = "windows",
                        ["arch"] = "x64",
                        ["url"] = $"https://api.slybrowser.test/v1/releases/artifacts/{_artifactSha256}.7z",
                        ["sha256"] = _artifactSha256,
