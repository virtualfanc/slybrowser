...
describe("standard CDP discovery", () => {
  ...
  it("provides authenticated standard discovery and rewrites WebSocket endpoints", async () => {
    ...
    const reconnect = await fetch(`${adapter.endpoint}/json/version`, {
      headers: { authorization: `Bearer ${token}` },
    });
    expect(reconnect.status).toBe(200);
    expect(await reconnect.json()).toEqual(discovery);
    expect((await fetch(`${adapter.endpoint}/json/list`)).status).toBe(401);
  });
});
