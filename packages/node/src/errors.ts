...
export type LicenseServiceErrorCode = typeof LICENSE_SERVICE_ERROR_CODES[number];

const LICENSE_SERVICE_ERROR_CODE_SET = new Set<string>(LICENSE_SERVICE_ERROR_CODES);

export function isLicenseServiceErrorCode(code: string): code is LicenseServiceErrorCode {
  return LICENSE_SERVICE_ERROR_CODE_SET.has(code);
}
