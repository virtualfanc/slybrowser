...
const MAX_BOOTSTRAP_HEARTBEAT_FAILURES = 10;
...
export function heartbeatDelayMilliseconds(
  ...
  return Math.round(baseSeconds * 1000);
}
...
class BootstrapHeartbeatController {
  ...
  async #heartbeat(): Promise<void> {
    ...
    try {
      ...
      if (this.#consecutiveFailures >= MAX_BOOTSTRAP_HEARTBEAT_FAILURES) {
        this.stop();
        return;
      }
      this.#schedule(heartbeatDelayMilliseconds(this.grant.heartbeatAfterSeconds));
...
async function installWithHeartbeat(
  ...
  const renew = async (): Promise<void> => {
    ...
    if (consecutiveFailures < MAX_BOOTSTRAP_HEARTBEAT_FAILURES) schedule();
  };
  schedule();
  try {
    const installation = await install();
    if (renewalFailure && consecutiveFailures >= MAX_BOOTSTRAP_HEARTBEAT_FAILURES) throw renewalFailure;
    return installation;
  } finally {
    stopped = true;
