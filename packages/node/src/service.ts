...
export interface LicenseFileReadOptions {
  ...
  licenseFileResolutionServiceUrl?: string;
...
export interface LicenseInfo {
  ...
  plan: "free" | "basic" | "pro" | "max" | "ultra";
  effectivePlan: "free" | "basic" | "pro" | "max" | "ultra";
...
function remoteErrorMessage(status: number, code: string): string {
...
const USER_LICENSE_FILE_FORMAT = "slybrowser-license-compact-v1";
const USER_LICENSE_FILE_FIELDS = new Set(["schemaVersion", "type", "format", "license"]);
const USER_LICENSE_FILE_MAX_COMPRESSED_BYTES = 64 * 1024;
const USER_LICENSE_FILE_MAX_DOCUMENT_BYTES = 256 * 1024;
...
function unwrapUserLicenseFile(value: unknown): unknown {
  const wrapper = requiredObject(value);
  if (
    wrapper.schemaVersion !== 1 ||
    wrapper.type !== "slybrowser-license" ||
    wrapper.format !== USER_LICENSE_FILE_FORMAT
  ) {
    return value;
  }
  if (Object.keys(wrapper).some((name) => !USER_LICENSE_FILE_FIELDS.has(name)) ||
      Object.keys(wrapper).length !== USER_LICENSE_FILE_FIELDS.size) {
    fail("authorization_invalid", "License file wrapper fields are invalid");
  }
  let serialized: string;
  try {
    serialized = gunzipSync(
      decodeBase64Url(requiredText(wrapper, "license", USER_LICENSE_FILE_MAX_COMPRESSED_BYTES * 2), USER_LICENSE_FILE_MAX_COMPRESSED_BYTES),
      { maxOutputLength: USER_LICENSE_FILE_MAX_DOCUMENT_BYTES },
    ).toString("utf8");
  } catch {
    fail("authorization_invalid", "License file wrapper payload is invalid");
  }
  if (Buffer.byteLength(serialized, "utf8") > USER_LICENSE_FILE_MAX_DOCUMENT_BYTES) {
    fail("authorization_invalid", "License file wrapper payload is too large");
  }
  try {
...
function parseEncryptedLicenseFile(value: unknown, options: LicenseFileReadOptions): LicenseAuthorization {
  const document = requiredObject(unwrapUserLicenseFile(value));
...
function localLicenseFileUnlockRequested(options: LicenseFileReadOptions): boolean {
  return options.licenseFilePassphrase !== undefined || options.licenseFileTrustedKeys !== undefined;
}

async function resolveEncryptedLicenseFileViaService(value: unknown, options: LicenseFileReadOptions): Promise<LicenseAuthorization> {
  const document = requiredObject(unwrapUserLicenseFile(value));
  if (document.schemaVersion !== 2 || document.type !== "slybrowser-license" ||
      document.audience !== "slybrowser-license-file" || document.channel !== "stable") {
    fail("authorization_invalid", "License file fields are invalid");
  }
  const documentServiceUrl = normalizeServiceUrl(requiredText(document, "serviceUrl"), options.allowInsecureLocalhost ?? false);
  assertTrustedAuthorizationOrigin(documentServiceUrl, options);
  const resolutionServiceUrl = normalizeServiceUrl(
    options.licenseFileResolutionServiceUrl ?? documentServiceUrl,
    options.allowInsecureLocalhost ?? false,
  );
  const fetchImplementation = options.fetch ?? fetch;
  let response: Response;
  try {
    response = await fetchImplementation(`${resolutionServiceUrl}/v1/license-files/authorization`, {
      method: "POST",
      headers: {
        "content-type": "application/json",
        "user-agent": "SlyBrowserNodeSDK/license-file",
      },
      body: JSON.stringify({ schemaVersion: 1, licenseFile: value }),
    });
  } catch {
    fail("license_service_unreachable", "License-file authorization service is unreachable", 0);
  }
  ...
  try {
    ...
    fail("license_service_invalid_response", "License-file authorization service returned invalid JSON", response.status);
  }
  if (!response.ok) {
    ...
    const details = error && typeof error === "object" && !Array.isArray(error)
      ...
      : {};
    const code = typeof details.code === "string" ? details.code : "license_file_authorization_failed";
    const message = typeof details.message === "string"
      ? details.message
      : "License-file authorization service rejected the license file";
    throw new LicenseServiceError(message, code, response.status, details);
  }
  const authorization = parseAuthorization(parsed, options.allowInsecureLocalhost ?? false);
  if (authorization.serviceUrl !== documentServiceUrl) {
    fail("license_service_invalid_response", "License-file authorization service URL does not match the license file", response.status);
  }
...
export async function readLicenseAuthorization(
  ...
  try {
    const document = parsed && typeof parsed === "object" && !Array.isArray(parsed)
      ? unwrapUserLicenseFile(parsed)
      : parsed;
    if (document && typeof document === "object" && !Array.isArray(document) &&
        (document as Record<string, unknown>).schemaVersion === 2) {
      if (!localLicenseFileUnlockRequested(options)) {
        return await resolveEncryptedLicenseFileViaService(parsed, options);
      }
      return parseEncryptedLicenseFile(parsed, options);
    }
    if (document && typeof document === "object" && !Array.isArray(document) &&
        (document as Record<string, unknown>).schemaVersion === 3) {
...
export class LicenseServiceClient {
  ...
  async licenseInfo(options: {
    ...
    const sdkVersion = options.sdkVersion ?? "0.2.0";
    ...
    const plans = new Set(["free", "basic", "pro", "max", "ultra"]);
  ...
  async createSession(options: {
    ...
    if (!new Set(["free", "basic", "pro", "max", "ultra"]).has(String(plan)) ||
  ...
  async #throwResponse(response: Response): Promise<never> {
    ...
    fail(
      code,
      remoteErrorMessage(response.status, code),
      response.status,
