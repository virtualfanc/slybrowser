...
test("release scripts hash both runtimes and produce a verifiable Ed25519 manifest", async (context) => {
  ...
  try {
    ...
    assert.equal(qualification.artifact.url, "https://api.slybrowser.com/v1/releases/artifacts/browser.zip");
    ...
    assert.equal(JSON.parse(await readFile(join(manifestRoot, "150.0.0.0.json"), "utf8")).signature.keyId, "release-test-v1");
...
test("remote release publisher is syntax-valid and scoped to the self-hosted API roots", async (context) => {
  if (process.platform !== "win32") {
    context.skip("PowerShell release publisher validation is Windows-specific");
    return;
  }
  const scriptPath = resolve("scripts/release/Publish-ReleaseBundleToServer.ps1");
  const script = await readFile(scriptPath, "utf8");
  assert.match(script, /RemoteArtifactRoot = '\/srv\/slybrowser\/releases\/artifacts'/);
  assert.match(script, /RemoteManifestRoot = '\/srv\/slybrowser\/releases\/manifests'/);
  assert.match(script, /AllowedArtifactHost = 'api\.slybrowser\.com'/);
  assert.match(script, /AllowedArtifactPathPrefix = '\/v1\/releases\/artifacts\/'/);
  assert.match(script, /scp/);
  assert.match(script, /ssh/);
  assert.equal(script.includes("$script:SshBaseArguments += @($script:SshTarget)"), false);
  const powershellPath = scriptPath.replaceAll("'", "''");
  execFileSync("powershell.exe", [
    "-NoProfile",
    "-Command",
    `$p='${powershellPath}'; [scriptblock]::Create((Get-Content -LiteralPath $p -Raw)) | Out-Null`,
  ]);
});
