...
const requiredThreats = new Set([
  ...
  "clock-rollback",
  "plan-feature-and-concurrency-gating",
...
const matrix = [
  ...
  },
  {
    ...
    status: "private-evidence-required",
    externalEvidenceId: "private-license-service:frontend-plan-tamper",
  },
  {
  ...
  },
  {
    id: "old-lease-replay",
    status: "private-evidence-required",
    externalEvidenceId: "private-license-service:old-lease-replay",
  },
  {
    id: "activation-ticket-replay",
    status: "private-evidence-required",
    externalEvidenceId: "private-license-service:activation-ticket-replay",
  },
  {
    id: "clock-rollback",
    status: "private-evidence-required",
    externalEvidenceId: "private-license-service:clock-rollback",
  },
  {
    id: "plan-feature-and-concurrency-gating",
    status: "private-evidence-required",
    externalEvidenceId: "private-license-service:plan-feature-and-concurrency-gating",
  },
  {
...
test("public security rows use public evidence and private-service rows use external receipt IDs", async () => {
  for (const item of matrix) {
    if (item.status === "private-evidence-required") {
      assert.match(item.externalEvidenceId ?? "", /^private-license-service:[a-z0-9-]+$/);
      assert.equal("evidence" in item, false, `${item.id} must not reference private repository paths`);
      continue;
    }
