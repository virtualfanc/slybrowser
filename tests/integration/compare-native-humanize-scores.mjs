...
function numericScore(language, report) {
  ...
  if (report.negativeCanary?.id !== "reject-mismatched-browser-driver" || report.negativeCanary?.status !== "PASS") {
    throw new Error(`${language} Native Humanize matrix did not pass the Browser/Driver mismatch canary`);
  }
...
async function main() {
  ...
  const languages = requiredLanguages.map((language) => {
    ...
    return {
      ...
      negativeCanary: reports[language].report.negativeCanary,
    };
  });
