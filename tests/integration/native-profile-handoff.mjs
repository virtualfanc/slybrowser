...
async function main() {
  ...
  const configuration = {
    ...
    profile: {
      ...
      geolocation: { ...expected.geolocation, permission: "allow" },
  ...
  try {
    ...
    const windowMetrics = await page.evaluate(async () => {
      ...
      const geolocation = await new Promise((resolveLocation) => {
        const timeout = setTimeout(() => resolveLocation(null), 5000);
    ...
    const webrtcCandidates = await page.evaluate(async () => {
      const peer = new RTCPeerConnection({ iceServers: [] });
      const candidates = [];
      try {
        peer.createDataChannel("profile-consistency");
    ...
    const checks = {
      ...
      webrtcProxyPolicy: webrtcRawAddresses.length === 0,
      geolocation: windowMetrics.geolocation
        && Math.abs(windowMetrics.geolocation.latitude - expected.geolocation.latitude) < 0.000001
        && Math.abs(windowMetrics.geolocation.longitude - expected.geolocation.longitude) < 0.000001,
      ...
      proxyBypass: Boolean(probe.headers()),
    };
    ...
    const result = {
      ...
      configuredParameterGroups: 29,
      runtimeCheckedGroups: [
        "proxy", "ua", "ua-full-version", "sec-ch-ua", "time-zone", "location",
        "ua-language", "screen", "webgl", "webgpu", "client-rects", "fonts", "webrtc", "cpu", "memory", "dnt",
      ],
      contextMatrix: {
        page: true,
        iframe: true,
        worker: true,
        webrtc: { candidates: webrtcCandidates, rawAddresses: webrtcRawAddresses },
