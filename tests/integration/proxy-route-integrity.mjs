...
function syntheticProxyCredential(label) {
  return `fixture-${label}-${randomUUID()}`;
}

const options = argumentsOf(process.argv.slice(2));
if (process.env.SLYBROWSER_ENABLE_FUTURE_PROXY_TESTS !== "1") {
  const result = {
    schemaVersion: 1,
    status: "SKIP",
    browser: options.browser,
    reason: "Proxy routing and GEO/WebRTC proxy behavior are not supported in the first SlyBrowser release.",
    enableWith: "SLYBROWSER_ENABLE_FUTURE_PROXY_TESTS=1",
  };
  if (options.output) {
...
const rejectedConfig = await privateConfig({
  proxy: {
    ...
    username: syntheticProxyCredential("user"),
    password: syntheticProxyCredential("password"),
    failClosed: true,
  },
