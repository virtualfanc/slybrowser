...
from slybrowser import launch_authorized_playwright
...
def main() -> int:
    ...
    parser.add_argument("--license-key-id", required=True)
    parser.add_argument("--license-public-key-hex", required=True)
    parser.add_argument("--release-key-id", required=True)
    parser.add_argument("--release-public-key-base64url", required=True)
    ...
    with sync_playwright() as playwright:
        browser = launch_authorized_playwright(
            playwright,
            args.authorization_file,
            license_trusted_keys={args.license_key_id: bytes.fromhex(args.license_public_key_hex)},
            release_trusted_keys={args.release_key_id: base64.urlsafe_b64decode(pad_base64(args.release_public_key_base64url))},
...
def pad_base64(value: str) -> bytes:
    return (value + "=" * ((4 - len(value) % 4) % 4)).encode("ascii")
