...
from slybrowser.licensed import install_authorized, launch_authorized
...
def main() -> None:
    ...
    parser.add_argument("--trusted-service-url")
    parser.add_argument("--canary-authorization-file")
    parser.add_argument("--canary-cache-root")
    parser.add_argument("--canary-trusted-service-url")
    parser.add_argument("--canary-license-key-id")
    parser.add_argument("--canary-license-public-key-hex")
    parser.add_argument("--canary-release-key-id")
    parser.add_argument("--canary-release-public-key-base64url")
    ...
    if options.authorization_file:
        for name in (
            ...
            "trusted_service_url",
            "canary_authorization_file",
            "canary_cache_root",
            "canary_trusted_service_url",
        ):
            if getattr(options, name) is None:
                raise ValueError(f"--{name.replace('_', '-')} is required with --authorization-file")
        options.canary_license_key_id = options.canary_license_key_id or options.license_key_id
        options.canary_license_public_key_hex = (
            options.canary_license_public_key_hex or options.license_public_key_hex
        )
        options.canary_release_key_id = options.canary_release_key_id or options.release_key_id
        options.canary_release_public_key_base64url = (
            options.canary_release_public_key_base64url or options.release_public_key_base64url
        )
    ...
    try:
        if options.authorization_file:
            session = launch_authorized(
                ...
                trusted_service_urls=(options.trusted_service_url,),
        ...
        session.close()
        session = None
        negative_canary = assert_artifact_mismatch_canary(options)
        ...
        report = {
            ...
            "negativeCanary": negative_canary,
...
def assert_artifact_mismatch_canary(options: argparse.Namespace) -> dict[str, str]:
    if options.authorization_file is None:
        return {"id": "not-configured", "status": "NOT_EVALUATED"}
    try:
        transport, artifact_downloader = canary_https_boundary()
        install_authorized(
            options.canary_authorization_file,
            license_trusted_keys={
                options.canary_license_key_id: bytes.fromhex(options.canary_license_public_key_hex)
            },
            release_trusted_keys={
                options.canary_release_key_id: base64url_decode(
                    options.canary_release_public_key_base64url
                )
            },
            allow_insecure_localhost=options.allow_insecure_localhost,
            trusted_service_urls=(options.canary_trusted_service_url,),
            cache_root=Path(options.canary_cache_root, "python"),
            platform="windows",
            arch="x64",
            update_kernel=False,
            transport=transport,
        ...
        if code != "artifact_runtime_hash_mismatch":
            raise
        return {
    ...
    raise AssertionError("Python SDK accepted the mismatched Browser/Driver canary")


def canary_https_boundary():
    ca_file = os.environ.get("SLYBROWSER_INTEGRATION_CANARY_CA_FILE")
    if not ca_file:
        return None, None
    ...
    def transport(method: str, url: str, headers, body: bytes | None):
    ...
    def artifact_downloader(url: str, headers, destination: Path) -> None:
    ...
    return transport, artifact_downloader
